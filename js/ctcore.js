var SECRET_KEYS = new Map();
SECRET_KEYS.set("https://blockchain.mmlab.edu.gr/SPIoT/server.php","jOhkR0z4HWQQmoqCMKdN2Jkn8QZeXwJALDBmYJH5gyc=");

async function createKeyIV(base64Token, uri)
{
    var hmacKey   = base64ToArrayBuffer(SECRET_KEYS.get(uri));
...
async function hmacToken(base64Token, uri)
{
    var hmacKey   = base64ToArrayBuffer(SECRET_KEYS.get(uri));
    var key       = await window.crypto.subtle.importKey("raw", hmacKey,{ name: "HMAC",hash: {name: "SHA-256"},}, false, ["sign", "verify"]);
    var hmac      = await window.crypto.subtle.sign({name: "HMAC",},key,base64ToArrayBuffer(base64Token));
    key           = await window.crypto.subtle.importKey("raw", hmac,{ name: "HMAC",hash: {name: "SHA-256"},}, false, ["sign", "verify"]);
    hmac          = await window.crypto.subtle.sign({name: "HMAC",},key,base64ToArrayBuffer(base64Token));
    return hmac;
}
